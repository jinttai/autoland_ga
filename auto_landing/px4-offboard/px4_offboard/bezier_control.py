...
class points():
    ...
    def time_calibrate(self):
        ...
        t = np.linalg.norm(self.xf[0:2] - self.xi[0:2]) / self.v_hor_max + np.linalg.norm(self.xf[2] - self.xi[2]) /self.v_ver_max
        flag = 0
        a_max = 0
        while flag == 0:
            point1 = self.xi
            ...
            point4 = self.xf
            point2 = np.array([self.xf[0], self.xf[1] ,self.xi[2] * 2/3 + self.xf[2] * 1/3])  
    ...
    def cmdloop_callback(self):
        if self.phase_check and self.loop_on:
            ...
            if self.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
                ...
                if self.delta_t + int(1/self.timer_period) < self.count-1 and np.linalg.norm(self.vehicle_position[2]-self.xf[2]) > 0.7 and self.detect:   # if receiving command from the bezier curve
